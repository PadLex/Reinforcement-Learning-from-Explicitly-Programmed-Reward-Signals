...
def convert_and_upload_dataset(load_dir: str):
    ...
    # TODO 5-10-24: now that val_set is an additional argument, the hash values for previously
    # generated datasets are invalid -- might need to add special logic based on whether
    # the use_val_set argument appears in run_args
...
if __name__ == "__main__":
    # convert_and_upload_dataset(load_dir="./logs/code-llama-13b-fitm-mask")

    tokenizer = load_model_and_tokenizer("./logs/code-llama-13b-fitm-mask", only_tokenizer=True)
    dataset = LudiiDataset(
        tokenizer=tokenizer,
        max_length=1024,
        dataset_type="fitm",
        mask_names=True,
        categories="board",
        sub_categories=["hunt", "race", "space", "war"],
        data_dir="./ludii_data/games",
        cache_dir="./caches",
        val_set=VALIDATION_GAMES
    )

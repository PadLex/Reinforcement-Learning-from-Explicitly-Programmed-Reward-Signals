...
if __name__ == '__main__':
    ...
    dataset = load_dataset("LudiiLMs/code-llama-13b-fitm-mask-base-data", split="val")

    games = dataset['base_game'][:20]

    evaluations = []
    with mp.Pool(2) as pool:
        with tqdm(total=len(games), desc="Evaluating fitnesses (MP)") as pbar:
            for evaluation in pool.imap(_get_fast_evaluation, games):
                evaluations.append(evaluation)
                pbar.update(1)


    print("Evaluation complete, validating...")
    for evaluation in evaluations:
        assert evaluation["compilable"]
    print("All games are compilable!")